...
|   |   |       |-- reserve.py - API scheda "Bank" (R5): GET reserve (ReserveView), /history (R6b: benchmark reserve_pct/btc_hold_pct/trading_pct), /settings; POST /settings (con freeze D22 e notifica), /transfer (in/out, notifica), /target-weights, /rebalance, /deploy. Errori dominio -> 400 con code; prezzi non disponibili -> 503.
...
|   |   |       |-- observability.py - endpoint admin-only Step 8 per log viewer dashboard con tail bounded, filtro livello/search, redazione valori sensibili e sorgente log derivata da `component` quando presente.
...
|   |   |   |-- agent_notifier.py - AgentNotifier: notifiche push tipizzate per trade spot/perp (idempotenti via set trade_id in RuntimeState), allarmi rischio (anti-spam), riepilogo giornaliero, eventi critici agente e eventi riserva "Bank" (notify_reserve_event: sweep/deploy/rebalance/transfer, opt-out via reserve_events); singleton get_agent_notifier(); preferenze utente in RuntimeState.
...
|   |   |   |-- archive.py - archiviazione dry-run in ArchivedRun, pulizia tabelle live e reset PortfolioState per reset analytics; include le tabelle riserva "Bank" (reserve_holdings/transactions/snapshots) nel payload e nella cancellazione, e azzera i contatori riserva su PortfolioState (_clear_reserve_counters).
|   |   |   |-- views.py - ViewService: spot_view, perp_view, global_view con PnL firmato, campi riserva "Bank" in GlobalView (D25: tradable_equity_usd = total − reserve_transferred_net_usd, total_portfolio_equity_usd, reserve_value/cash/pnl/fees, total_portfolio_pnl_pct; valore dall'ultimo ReserveSnapshot con fallback al costo; fee riserva sommate a total_fees_usd; floor guardrail su tradable_equity; volatility_budget D28 = vol giornaliera + max drawdown trading vs portafoglio totale, insufficient_data come lo Sharpe), history Spot/Perp allineata ai trade chiusi con PnL, conteggio trade giornaliero invariato per mercato, Bot Day condiviso dal primo ordine Spot/Perp registrato, esposizione a margine, fee aggregate, position_id nella history Perp inclusi eventi Smart SL `ssl_`, entry storica Perp coerente per chiusure parziali, risk-off spot e Sharpe ratio guarded.
...
|   |   |   |-- notification_prefs.py - NotificationPreferences (6 toggle: spot/perp/risk/summary/critical/reserve_events) e NotificationPreferencesResponse con campo source (default/persisted).
...
|       |-- App.tsx - viste Overview, Spot, Global, Bank (scheda Riserva R9: BankPanel con metriche/posizioni/pesi/movimenti/azioni admin/impostazioni), Health con Data Coverage filtrabile, Wallet, Logs, Settings, Onboarding, Markets, Support con archivio, Export, metriche Trade Day e Bot Day in Spot/Perp, metriche riserva + volatility budget nel GlobalPanel, toggle reserve_events nel NotificationPrefsPanel, backend URL normalizzato nel form per non usare la porta dashboard come API, formatter micro-prezzi con soglia full-decimal sotto 0.000001 e dettaglio trade con grafico/fee/margine/riferimento SL Perp; nei grafici trade le candele fuori dalla finestra di posizione sono attenuate, il marker SL ref cade sul prezzo SL effettivo e la linea ref evita la candela; le history Spot/Perp hanno filtri e selettore 20/50/100/Tutti per pagina, mentre il Settings espone i buffer Min/Max20 Spot/Perp, il toggle margine fisso Perp e i toggle Time Stop Spot/Perp con durata in ore.
...
| Log viewer admin-only | I log backend sono esposti solo tramite endpoint admin con limite righe e redazione pattern sensibili; la dashboard non legge file locali, non espone path reali e classifica il tail per priorità operativa, categoria, ricerca e azione suggerita. |
| Alert critici agente | Le push FCM di engine health, inclusi `storage_error` e `scan_failures`, sono evidenziate nel Log Viewer come critical/actionable se rientrano nel tail recente; l'endpoint log supporta filtro `search`, ma non esiste ancora una inbox persistente separata dai log. |
