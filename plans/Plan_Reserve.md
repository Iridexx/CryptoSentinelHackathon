...
| **D31** | **Impatto uscite sulle statistiche** | Analisi (2026-08-30): i transfer verso la riserva **non** contano come perdite — curva equity, drawdown, Sharpe, P&L, win-rate sono su `initial + realized + unrealized` che il transfer non tocca; nessun `EquityAdjustment` (non ribaselina la storia). **Fix R4c**: (a) `exposure_pct` e `daily_loss_limit_used_pct` in `_update_portfolio_state` passano a `tradable_equity`; (b) colonna `total_portfolio_equity_usd` su `PnlSnapshot` (fino a R6 = `total_equity`, poi valore riserva MTM reale); (c) endpoint `/equity-curve` espone `portfolio_equity_usd` per punto. **Toggle "Solo Trading / Portafoglio Totale"** sul grafico + peak/drawdown a livello portafoglio = R6b (serie) + R7 (UI). |
...
| ~~R4c~~ | **FATTO** 2026-08-30 — `_update_portfolio_state` calcola `exposure_pct`/`daily_loss_limit_used_pct` su `tradable_equity`; colonna `total_portfolio_equity_usd` su `PnlSnapshot` + migrazione; `_snapshot_portfolio_hourly` la popola (riserva al costo fino a R6); `/equity-curve` espone `portfolio_equity_usd`/`portfolio_pnl_*` per punto (solo global). `test_reserve_r4c_stats.py` (3 test). Report `report_reserve_r4c_stats.md`. |
| ~~R4b~~ | **FATTO** 2026-08-30 — `persistence/archive.py`: `archive_dry_run_records` e `reset_all_data` archiviano e cancellano `reserve_holdings/transactions/snapshots` + `_clear_reserve_counters` azzera i campi riserva su `PortfolioState` (sempre quando si cancellano le righe, per non lasciare `transferred_net` orfano). `test_reserve_archive.py` (4 test). Report `report_reserve_r4b_archive.md`. |
| ~~R5~~ | **FATTO** 2026-08-30 — `api/routes/reserve.py` (GET `""`/`/history`/`/settings`, POST `/settings`/`/transfer`/`/target-weights`/`/rebalance`/`/deploy`) + router registrato + `domain/reserve/pricing.py` (`build_reserve_service` con prezzi Binance spot batch) + chiavi i18n en/it + `test_reserve_api.py` (7 test). Report `report_reserve_r5_api.md`. |
| ~~R6~~ | **FATTO** 2026-08-30 — `AgentService._reserve_tick` nel `slow_tick`: sweep → deploy → rebalance (se `auto_rebalance`) → `ReserveSnapshot` orario (MTM reale via `build_reserve_service`); `hard_stop` salta le azioni ma non lo snapshot; `_snapshot_portfolio_hourly` ora usa il valore MTM dell'ultimo `ReserveSnapshot`. `AgentNotifier.notify_reserve_event` + `NotificationPreferences.reserve_events` (D23); anche il transfer via API notifica. `test_reserve_slow_tick.py` (5 test). Report `report_reserve_r6_slow_tick.md`. |
| ~~R6b~~ | **FATTO** 2026-08-30 — `/agent/reserve/history` espone `reserve_pct` / `btc_hold_pct` (riuso `_btc_benchmark`) / `trading_pct` (serie `PnlSnapshot` allineata per offset orario); `VolatilityBudgetView` + `_volatility_budget` (vol giornaliera + max drawdown, trading vs portafoglio totale, `insufficient_data` finché < 7 rendimenti giornalieri) in `GlobalView`. `test_reserve_benchmark.py` (4 test). Report `report_reserve_r6b_benchmark.md`. |
| ~~R7a~~ | **FATTO** 2026-08-30 — `agentApi.ts` (client reserve + tipi, campi riserva/volatility in `GlobalView`, `portfolio_*` in equity-curve); pulsante "🏦 Bank" full-width in `AgentTab`; `BankPane` (header valore/P&L/capacità/deploy, grafico multi-linea %↔$, pesi vs target, posizioni, movimenti, azioni admin transfer/deploy/rebalance); `GET /agent/reserve/transactions`. tsc OK, nessun nuovo errore eslint. Report `report_reserve_r7a_bankpane.md`. |
| R7b | Setup › Bank sub-tab (pesi + sweep/deploy/cooldown/toggle) + GlobalPane (stat Bank, tradabile vs totale, volatility budget) + toggle equity "Solo Trading / Portafoglio Totale" + `SettingsTab` toggle `reserve_events` |
